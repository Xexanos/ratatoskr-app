...
object WireFixtures {
    ...
    /**
     * A `Session` response body. [extraJson] injects raw additional members (e.g.
    ...
    fun sessionJson(
        ...
        return """{"itemId":"i1","speakerId":"s1","state":"${esc(state)}","positionSeconds":$positionSeconds,""" +
            """"durationSeconds":$durationSeconds,"updatedAt":"2026-07-05T12:00:00Z"$extra}"""
    }

    /** An `AuthSession` response body, as returned by /v2 login: one opaque token plus the user. */
    fun authSessionJson(
        token: String = "t1",
        username: String = "alex",
    ): String =
        """{"token":"${esc(token)}","user":{"id":"7","username":"${esc(username)}"}}"""
    ...
    /** A `GET /v2/speakers` response body (a JSON array of speakers). */
     ...
     * optional). [coverUrl] is origin-relative on the wire
     * (e.g. `/v2/library/items/i1/cover`); the client resolves it against its base URL.
     */
    ...
    /** A `GET /v2/library/items` response body (a `LibraryItemPage`). */
    fun libraryPageJson(
    ...
    /** A `GET /v2/library/in-progress` response body (a `LibraryItemList` - no cursor). */
